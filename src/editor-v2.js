...
let data=defaultData(), selectedId=null, selectedThree=null, routeGhost=null,routeGhostInfo=null,currentTileId=null,floorPaint=false,eraseFloorMode=false,selectedDecalId=null,draggingDecal=false,decalEdit=false,decalTransform=null,decalRepeatMode=false,lastFloorPointer=null,referenceEdit=false,referenceDrag=null;
...
async function instantiateDecal(rec){
  ...
  const mat=new THREE.MeshBasicMaterial({map:tex,transparent:rec.transparent!==false,side:THREE.DoubleSide,depthWrite:false,depthTest:false});
  const m=new THREE.Mesh(new THREE.PlaneGeometry(1,1),mat);m.rotation.set(-Math.PI/2,0,THREE.MathUtils.degToRad(rec.rotation||0));m.position.set(rec.x||0,.015,rec.z||0);m.scale.set(rec.scaleX||1,rec.scaleY||1,1);m.renderOrder=600;m.userData.decalId=rec.id;decalGroup.add(m);decalRuntime.set(rec.id,m);return m;
}
...
async function duplicateDecal(){
  ...
  const c=structuredClone(d);c.id=uid('decal');
  data.decals.push(c);await instantiateDecal(c);setDecalSelected(c.id);saveSoon();
}
async function repeatDecal(dir){
  const d=selectedDecal();if(!d||d.locked)return;
  const c=structuredClone(d);c.id=uid('decal');
  const a=THREE.MathUtils.degToRad(d.rotation||0),ux=Math.cos(a),uz=Math.sin(a),vx=-Math.sin(a),vz=Math.cos(a);
  if(dir==='right'){c.x+=ux*d.scaleX;c.z+=uz*d.scaleX}
  else if(dir==='left'){c.x-=ux*d.scaleX;c.z-=uz*d.scaleX}
  else if(dir==='up'){c.x+=vx*d.scaleY;c.z+=vz*d.scaleY}
  else if(dir==='down'){c.x-=vx*d.scaleY;c.z-=vz*d.scaleY}
  c.x=+c.x.toFixed(4);c.z=+c.z.toFixed(4);
  data.decals.push(c);await instantiateDecal(c);setDecalSelected(c.id);saveSoon();
}
function cancelDecalTransform(){
  if(!decalTransform)return;
  const d=selectedDecal();
  if(d&&decalTransform.start){Object.assign(d,decalTransform.start);const m=decalRuntime.get(d.id);if(m){m.position.set(d.x,.015,d.z);m.rotation.set(-Math.PI/2,0,THREE.MathUtils.degToRad(d.rotation||0));m.scale.set(d.scaleX,d.scaleY,1)}}
...
function updateDecalTransform(p,event=null){
  ...
  if(t.mode==='move'){
    ...
    let rotation=t.start.rotation+delta;
    if(event?.ctrlKey||event?.metaKey)rotation=Math.round(rotation/45)*45;
    d.rotation=+rotation.toFixed(2);
  ...
  const m=decalRuntime.get(d.id);if(m){m.position.set(d.x,.015,d.z);m.rotation.set(-Math.PI/2,0,THREE.MathUtils.degToRad(d.rotation||0));m.scale.set(d.scaleX,d.scaleY,1)}
...
function renderFloorControls(){
  ...
  <div class="floor-layer-card decal-card ${decalEdit?'editing':''}"><b>Decals</b><button id="decalEditBtn" class="${decalEdit?'active':''}">${decalEdit?'✓ Edición de decals':'✥ Edición de decals'}</button><label class="file-button wide">Agregar JPG / PNG<input id="decalUpload" type="file" accept="image/jpeg,image/png,image/webp"></label><small>En modo edición: click selecciona · G mover · R rotar · S escalar · P repetir · X/Y restringir · Ctrl+D duplicar · Delete borrar. En R, Ctrl snapea a 45°. Ctrl+click coloca la decal elegida.</small><div id="decalLibraryV23">${lib}</div>${d?`<div class="decal-selected-panel"><b>Seleccionada</b><label><input id="decalLock" type="checkbox" ${d.locked?'checked':''}> Bloquear posición</label><label><input id="decalKeepAspect" type="checkbox" ${d.keepAspect!==false?'checked':''}> Mantener proporción original</label><div class="row"><label>Ancho <input id="decalSX" type="number" step=".1" value="${(+d.scaleX).toFixed(3)}"></label><label>Alto <input id="decalSY" type="number" step=".1" value="${(+d.scaleY).toFixed(3)}"></label></div><small>Rotación: ${(+d.rotation||0).toFixed(1)}° · durante R: Ctrl = snap 45°</small><button id="decalRepeatBtn" class="${decalRepeatMode?'active':''}">P · Repetir decal</button>${decalRepeatMode?`<div class="decal-repeat-pad"><span></span><button data-repeat-dir="up">↑</button><span></span><button data-repeat-dir="left">←</button><button data-repeat-dir="down">↓</button><button data-repeat-dir="right">→</button></div><small>Las flechas siguen los ejes rotados de la decal.</small>`:''}<div class="row"><button id="duplicateDecal">Ctrl+D Duplicar</button><button id="deleteDecal">Eliminar decal</button></div></div>`:''}</div>
  ...
  [['baseRX','base','repeatX'],['baseRY','base','repeatY'],['dirtRX','dirt','repeatX'],['dirtRY','dirt','repeatY']].forEach(([id,l,k])=>$('#'+id).onchange=async e=>{data.floor[l][k]=Math.max(1,+e.target.value||1);await rebuildFloorLayers();await rebuildDecals();saveSoon()});
  $('#baseRot').onchange=async e=>{data.floor.base.rotate90=e.target.checked;await rebuildFloorLayers();await rebuildDecals();saveSoon()};$('#dirtRot').onchange=async e=>{data.floor.dirt.rotate90=e.target.checked;await rebuildFloorLayers();await rebuildDecals();saveSoon()};
  ...
  $('#decalEditBtn').onclick=()=>{decalEdit=!decalEdit;decalTransform=null;decalRepeatMode=false;draggingDecal=false;orbit.enabled=true;if(!decalEdit)clearDecalSelection();renderFloorControls();$('#modeBadge').textContent=decalEdit?'DECALS':'OBJETO'};
...
renderer.domElement.addEventListener('pointermove',e=>{
  ...
  if(decalTransform&&fp){updateDecalTransform(fp,e);return}
  if(referenceDrag){
  ...
  if(!draggingDecal||!selectedDecalId)return;const d=selectedDecal();if(!d||d.locked)return;const p=floorPointFromEvent(e);if(!p)return;d.x=+p.x.toFixed(4);d.z=+p.z.toFixed(4);const m=decalRuntime.get(d.id);if(m)m.position.set(d.x,.015,d.z);saveSoon()
});
